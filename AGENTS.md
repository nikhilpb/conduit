...
- The backend runs a single agent only. No router or specialist hierarchy is active in the current implementation.
...
- The root agent is responsible for the final user-facing response.
...
- `src/conduit/tool_call_utils.py`
  ...
  - Internal tool filtering hides `adk_request_confirmation` plumbing from app-facing transcripts, websocket events, and context estimates.
...
- No general multi-agent router or specialist set is implemented yet.
