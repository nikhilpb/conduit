...
`config/scheduled_sessions.yaml` configures optional headless scheduled runs. Each entry defines `id`, `schedule` (5-field cron in UTC), `model`, `seed_query`, and `allowed_tools`. Each trigger creates a fresh scheduled session in the main SQLite session history and injects the scheduler fire time in UTC as the per-turn current-time context.

The repo currently ships with two scheduled jobs: `iran-us-conflict-news` runs daily at `08:00 UTC` using `claude-opus-4-6` plus `web_search` and `web_fetch`, and `test-hi` runs daily at `00:00 UTC` using `gemini-3-flash-preview`.
